          ...
          <div className="p-4 border-b-2 border-[#0a0a0a] shrink-0">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <MessageCircle className="w-4 h-4 text-[#2E6A4A]" />
            ...
            <div className="border-2 border-[#e0e0dc] p-3">
              ...
              {cible.question_cle && (
                <p className="text-[11px] text-[#2E6A4A] mt-2 border-l-2 border-[#2E6A4A] pl-2 leading-relaxed">{cible.question_cle}</p>
              )}
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        {obs.indice_juge && (
                          <span className="text-[9px] bg-[#D3E4D7] text-[#2E6A4A] px-1.5 py-0.5 border border-[#2E6A4A] uppercase tracking-wider">
                        ...
                        {obs.photos!.map((photo, i) => (
                          <button
                            ...
                            className="shrink-0 w-16 h-16 sm:w-20 sm:h-20 overflow-hidden border-2 border-[#e0e0dc]
                                       hover:border-[#2E6A4A] transition-all active:scale-95"
                          >
                            <img
                              src={photo}
                              alt={`Photo ${i + 1}`}
                    ...
                    {obs.validation_fields?.criteres_mal_evalues && obs.validation_fields.criteres_mal_evalues.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {obs.validation_fields.criteres_mal_evalues.map((c) => (
                          <span key={c} className="text-[9px] uppercase tracking-wider bg-[#D3E4D7] text-[#2E6A4A] px-2 py-0.5 border border-[#2E6A4A]">{c}</span>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#e0e0dc]">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-[#D3E4D7] transition-colors group"
                          title="Confirmer"
                        >
                          <ThumbsUp className="w-3.5 h-3.5 text-[#ccc] group-hover:text-[#2E6A4A]" />
                        </button>
                        <span className={`text-[10px] min-w-[20px] text-center font-mono ${score > 0 ? 'text-[#2E6A4A]' : score < 0 ? 'text-red-600' : 'text-[#ccc]'}`}>
                          {score > 0 ? `+${score}` : score}
                        </span>
                        <button
