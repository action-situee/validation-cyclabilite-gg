...
export function HoveredSegmentCard({
  ...
  const renderAttributes = (classKey: BikeMetricKey) => {
    ...
    return (
      <div className="p-3 space-y-2 bg-[#F3F4F6] border-t border-[#e0e0dc]">
        {bikeClass.attributes.map((attribute) => {
          ...
          return (
            <div key={attribute.technicalName} className="bg-white border border-[#e0e0dc] p-2.5">
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <span className="text-[10px] text-[#0a0a0a]">{attribute.name}</span>
                <span className="text-[10px] text-[#5c5c5c] font-mono">
                  {formatScore(Number.isFinite(value) ? value : null)}
                </span>
              </div>
              <div className="h-1.5 bg-[#ece9e4] overflow-hidden">
                <div
                  className="h-full"
                  style={{
                    width: `${safeValue * 100}%`,
          ...
          return (
            ...
            >
              <div className="flex items-stretch">
                <button
                  type="button"
                  onClick={() => onMetricChange(bikeClass.key)}
                  className={`w-10 shrink-0 flex items-center justify-center transition-colors ${
                ...
                <button
                  type="button"
                  onClick={() => setExpandedClassKey((previous) => previous === bikeClass.key ? null : bikeClass.key)}
                  className="flex-1 p-3 text-left transition-colors hover:bg-[#E5EEE6]"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[11px] text-[#0a0a0a]">{bikeClass.label}</p>
                      <p className="text-[9px] text-[#5c5c5c] mt-0.5">{bikeClass.description}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[11px] text-[#2E6A4A] font-mono">
                        {formatScore(classValue)}
                      </span>
                      <ChevronRight
                        className={`w-3.5 h-3.5 text-[#999] transition-transform ${isExpanded ? 'rotate-90' : ''}`}
                      />
                    </div>
                  </div>
                </button>
